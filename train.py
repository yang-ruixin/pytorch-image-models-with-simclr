...
Modified by YANG Ruixin for multi-label classification with SimCLR
2021/09/07
...
# ================================
parser.add_argument('-t', '--temperature', type=float, default=0.5, metavar='N',
                    help='see appendix B.7.: Optimal temperature under different batch sizes')
parser.add_argument('-wls', '--simclr-loss-weight', type=float, default=0.5, metavar='N')
parser.add_argument('-wlc', '--classification-loss-weight', type=float, default=0.5, metavar='N')
# ================================
...
def main():
    ...
    criterion_for_simclr = NT_Xent(args.batch_size, args.temperature, args.world_size)  # use for SimCLR loss
    # ================================
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics, lr, total_simclr_loss, total_classification_loss = train_one_epoch(
                args.simclr_loss_weight, args.classification_loss_weight, criterion_for_simclr,  # add 3 arguments for SimCLR
                num_of_data_train, epoch, model, loader_train, optimizer, train_loss_fn, args,
                lr_scheduler=lr_scheduler, saver=saver, output_dir=output_dir,
                amp_autocast=amp_autocast, loss_scaler=loss_scaler, model_ema=model_ema, mixup_fn=mixup_fn)

            # ================================
            _logger.info('Loss of SimCLR after this epoch: {0}'.format(total_simclr_loss / len(loader_train)))
            _logger.info('Loss of classification after this epoch: {0}'.format(total_classification_loss / len(loader_train)))
            _logger.info(' ')
            # ================================
            ...
            update_summary(
                epoch, train_metrics, eval_metrics, lr,
                total_simclr_loss / len(loader_train), total_classification_loss / len(loader_train),
...
def train_one_epoch(
        simclr_loss_weight, classification_loss_weight, criterion_for_simclr,  # add 3 arguments for SimCLR
    ...
    # ================================
    total_loss = total_simclr_loss = total_classification_loss = 0
    ...
    for batch_idx, ((input, input_j), target) in enumerate(loader):  # for simclr
        last_batch = batch_idx == last_idx
        data_time_m.update(time.time() - end)
        if not args.prefetcher:
            # input, target = input.cuda(), target.cuda()
            input, input_j, target = input.cuda(), input_j.cuda(), target.cuda()
            if mixup_fn is not None:
                input_j, _ = mixup_fn(input_j, target)
                input, target = mixup_fn(input, target)
        if args.channels_last:
            input = input.contiguous(memory_format=torch.channels_last)
            input_j = input_j.contiguous(memory_format=torch.channels_last)

        with amp_autocast():
            # output = model(input)

            h_i, h_j, z_i, z_j, output = model.forward_train(input, input_j)
            # hi shape is batchSize * num_features
            # zi shape is batchSize * projection_dim
            # output['color'] shape is batchSize * numberOfDifferentColor

            # ================================
            simclr_loss = criterion_for_simclr(z_i, z_j)
            classification_loss = model.get_loss(loss_fn, output, target)

            loss = simclr_loss * simclr_loss_weight + classification_loss * classification_loss_weight
            total_simclr_loss += simclr_loss.item() * simclr_loss_weight
            total_classification_loss += classification_loss.item() * classification_loss_weight
    ...
    return OrderedDict([('loss', losses_m.avg)]), lr, total_simclr_loss, total_classification_loss
